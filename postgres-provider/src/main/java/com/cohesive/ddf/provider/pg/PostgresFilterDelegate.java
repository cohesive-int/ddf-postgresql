...
public class PostgresFilterDelegate extends FilterDelegate<SQLHolder> {
    ...
    private static final Logger logger = LoggerFactory.getLogger( PostgresFilterDelegate.class );
    ...
    public SQLHolder xpathExists( String xpath ) {
        ...
        xpath = normalizeDefaultNamespaces(
                normalizeDefaultNamespaces( normalizeDefaultNamespaces( xpath, new StringBuilder(), '[' ), new StringBuilder(), '/' ), new StringBuilder(), '@' );
        ...
        // "xpath_exists( '"+xpath+"', xml(catalog_metadata), "+namespaceHandler.getNamespacesAsString()+" )"
        // );
    }
    ...
    public SQLHolder xpathIsLike( String xpath, String pattern, boolean isCaseSensitive ) {
        ...
        if ( index >= 0 ) {
            xpath = xpath.replace( "/@", "[@" );
            xpath += "=\"" + pattern + "\"]";
            returnHolder = new SQLHolder( "array_length(xpath( '" + xpath + "', xml(catalog_metadata), " + namespaceHandler.getNamespacesAsString()
                    + " ),1) is not null " );
            // returnHolder = new SQLHolder(
            // "xpath_exists( '"+xpath+"', xml(catalog_metadata), "+namespaceHandler.getNamespacesAsString()+" )"
            // );
        } else {
            List<SQLHolder> holders = new ArrayList<SQLHolder>();
            holders.add( propertyIsLike( Metacard.ANY_TEXT, pattern, isCaseSensitive ) );
            holders.add( new SQLHolder( "array_length(xpath( '" + xpath + "', xml(catalog_metadata), " + namespaceHandler.getNamespacesAsString()
                    + " ),1) is not null " ) );
            // holders.add( new SQLHolder(
            // "xpath_exists( '"+xpath+"', xml(catalog_metadata), "+namespaceHandler.getNamespacesAsString()+" )"
            // ) );
        ...
        if ( wkt.startsWith( GEOMETRYCOLLECTION ) ) {
            try {
                ...
                for ( int i = 0; i < numGeos; i++ ) {
                    ...
                    sqlHolders.add( new SQLHolder( columnName + "&& ST_GeomFromEWKT('SRID=4326;" + wkt + "') AND " + operation
                            + "( ST_GeomFromEWKT('SRID=4326;" + wkt + "'), " + columnName + ")" ) );
                }
            ...
            return new SQLHolder( columnName + "&& ST_GeomFromEWKT('SRID=4326;" + wkt + "') AND " + operation + "( ST_GeomFromEWKT('SRID=4326;" + wkt + "'), "
                    + columnName + ")" );
        }
    }
    ...
    private SQLHolder handleDistanceWithin( String propertyName, String wkt, double distance ) {
        ...
        SQLHolder holder = new SQLHolder( " ST_DWithin( " + name + ",  ST_GeomFromEWKT('SRID=4326;" + wkt + "'), " + metersToDegrees( distance ) + " )"
                + " AND " + "ST_Distance_Sphere(" + name + ",  ST_GeomFromEWKT('SRID=4326;" + wkt + "')) < '" + distance + "'" );
    ...
    private SQLHolder handlePropertyIsLike( String propertyName, String searchPhrase, boolean isCaseSensitive ) {
        ...
        if ( Metacard.ANY_TEXT.equals( propertyName ) ) {
            ...
            // Need to add in extra logic here to handle if it is not querying
            ...
            if ( query == null ) {
                ...
                query = new SQLHolder( mappedPropertyName + (isCaseSensitive ? " like '%" : " ilike '%") + searchPhrase.replace( "*", "%" ) + "%'" );
            }
